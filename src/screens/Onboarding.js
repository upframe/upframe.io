...
export default class Onboarding extends Component {
  ...
  viewPasswordTab = res => {
    this.setState({ currentTab: 2, propsToPass: res })
  }
  ...
  renderCurrentTab = () => {
    if (this.state.currentTab === 1) {
      ...
      return (
        <OnboardingPasswordTab
          next={this.viewAllDoneTab}
          data={this.state.propsToPass}
        />
      )
  ...
  render() {
    return <div>{this.renderCurrentTab()}</div>
  }
}
