...
const renderContext = (mockChangeSearchWidth,mockLoggedIn,mockSetProfilePic,mockLoggedOut) => { 
    
    return render(
        <Router>
            <AppContext.Provider value={{changeSearcBarhWidth:mockChangeSearchWidth,loggedIn:mockLoggedIn,user:mockSetProfilePic,logout:mockLoggedOut}} >
                <Navbar />
            </AppContext.Provider>
        </Router>
    )
...
describe( "testing user logged state", () => {
    ...
    it("test if user can see profile picture when he is logged in", () => {
        const {getByAltText} = renderContext(mockChangeSearchWidth,mockLoggedIn,mockSetProfilePic)
        const picAltText = getByAltText('profile picture')
        expect(picAltText).toBeTruthy()     
    }) 

    it("test if user can log out", () => {
        const {getByAltText,getByText} = renderContext(mockChangeSearchWidth,mockLoggedIn,mockSetProfilePic,mockLoggedOut)
        const picAltText = getByAltText('profile picture')
...
describe( "testing drop down", () => {
    ...
    it("dropdown is closing ", () => {
        const {getByAltText,getByPlaceholderText} = renderContext(mockChangeSearchWidth,mockLoggedIn,mockSetProfilePic)
        const picAltText = getByAltText('profile picture')
        fireEvent.click(picAltText)
        fireEvent.click(picAltText)
        const getDropDown = document.getElementsByClassName(styles.showMenu)[0]
