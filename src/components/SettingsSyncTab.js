...
export default class SettingsSyncTab extends Component {
    ...
    Api.getUserInfo().then(res => {
      if (res.user.googleAccessToken !== '') {
        ...
        this.getCalendarList(res.user.googleAccessToken).then(res => {
          let newCalendarsList = res.items
            .filter(element => {
              return (
                !element.id.includes('#holiday@group.v.calendar.google.com') &&
                !element.id.includes('#contacts@group.v.calendar.google.com')
              )
            })
            .map(element => {
              return {
            ...
            this.setState({
              freeSlotsSaved: res.slots.map(unconvertedSlot => {
                return {
                  ...
                  start: moment
                    .utc(unconvertedSlot.start)
                    .utcOffset(-timeOffset)
                    .toDate(),
                  end: moment
                    .utc(unconvertedSlot.end)
                    .utcOffset(-timeOffset)
                    .toDate(),
                  isMine: true,
                }
  ...
  calendarVisibilityChange = event => {
    //After displaying the calendars we want to show their visibility according to checks
    let newCalendars = this.state.calendars
    newCalendars = this.state.calendars.map(element => {
      //Here we find which calendar got checked
      return {
        id: element.id,
        summary: element.summary,
        checked:
    ...
    this.getCalendarEvents(newCalendars).then(data => {
      //Here we fetch the events for all the checked calendars
      this.setState({
  ...
  deleteFreeSlot = event => {
    let isUnsavedFreeSlot = this.state.freeSlotsUnsaved.some(
      slot => slot.id === event.id
    )
    if (isUnsavedFreeSlot) {
      this.setState({
        freeSlotsUnsaved: this.state.freeSlotsUnsaved.filter(
          slot => slot.id !== event.id
        ),
      })
    } else {
      ...
      this.state.freeSlotsSaved.forEach(slot => {
        if (slot.id === event.id) {
          //evento para remover adicionamos ao delete
          newFreeSlotsToDelete.push(slot)
        } else {
          //outro evento, deixamos estar
  ...
  addFreeSlot = slot => {
    ...
    if (!stop) {
      ...
      let allFreeSlots = [
        ...this.state.freeSlotsUnsaved,
        ...this.state.freeSlotsSaved,
      ]
      let leng = allFreeSlots.length
      for (let x = 0; x < leng && !stop; x++) {
        if (
          slot.end > allFreeSlots[x].start &&
          slot.end < allFreeSlots[x].end
        ) {
          //The end in inside another slot
          stop = true
        } else if (
          slot.start > allFreeSlots[x].start &&
          slot.start < allFreeSlots[x].end
        ) {
          //The beginning is inside another slot
          stop = true
        } else if (
          slot.end >= allFreeSlots[x].end &&
          slot.start <= allFreeSlots[x].start
        ) {
          //THis slot contains another one
          stop = true
        }
      }
    }
  ...
  getCalendarEvents = calendarList => {
    let checkedCalendars = calendarList.filter(calendar =>
      calendar.checked ? calendar : null
    )
    let calendarIds = checkedCalendars.map(calendar => calendar.id)
    let calendarPromises = calendarIds.map(calendarId =>
      this.calendarEvents(calendarId)
    )
    let eventsPromise = Promise.all(calendarPromises)
      .then(done => {
        let allEvents = []
  ...
  calendarEvents = calendarId => {
    ...
    customHeaders.append(
      'Authorization',
      'Bearer ' + this.state.googleAccessToken
    )
    return fetch(
  ...
  saveFreeSlots = () => {
    ...
    Api.addFreeSlots(
      ...
      if (res.ok === 1) {
        ...
        Api.getFreeSlots(nowDate, limitDate).then(
          res => {
            // UI user feedback
          ...
          () => {
            mixpanel.track(
              '[Free Slots] ' + this.context.user.name + ' - created free slots'
            )
          }
        )
      } else {
      ...
      return (
        <div id="settings-synctab">
          <div id="controls">
            ...
            <button className="btn btn-secondary" onClick={this.googleSync}>
              Google Sync
            </button>
          </div>
      ...
      return (
        <React.Fragment>
          <div id="settings-synctab">
                    ...
                    <div key={element.id} id="calendar-item">
                      <label key={element.id} htmlFor={element.id}>
                        <input
                          ...
                          type="checkbox"
                          id={element.id}
                          onChange={this.calendarVisibilityChange}
            ...
            <div id="controls">
              <button className="btn btn-secondary" onClick={this.unlinkGoogle}>
                Unlink Google
              </button>
            </div>
            <div id="calendar" className="grid">
